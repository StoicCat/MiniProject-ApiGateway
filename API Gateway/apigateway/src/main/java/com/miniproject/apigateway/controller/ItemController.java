...
import com.miniproject.apigateway.model.request.UpdateInStockItemRequest;
...
public class ItemController {
		...
		return healthCheckService.checkBackendHealth().flatMap(isAvailable -> {
			if (Boolean.TRUE.equals(isAvailable)) {
				return webClientBuilder.build().get()
						.uri(Environment.SERVICE_URL + "supplier-item/get-all-supplier-item")
						.headers(headers -> headers.setBearerAuth(authorizationHeader)).retrieve()
	...
	public Mono<ApiResponse<SupplierItemResponse>> getSupplierItem(@RequestHeader("IniBukanKunci") String authorizationHeader,
			@PathVariable String id) {
		return healthCheckService.checkBackendHealth().flatMap(isAvailable -> {
			if (Boolean.TRUE.equals(isAvailable)) {
				return webClientBuilder.build().get()
						.uri(Environment.SERVICE_URL + "supplier-item/get-supplier-item-by-id/" + id)
						.headers(headers -> headers.setBearerAuth(authorizationHeader)).retrieve()
	...
	public Mono<ApiResponse<String>> addSupplierItems(@RequestHeader("IniBukanKunci") String authorizationHeader,
			@RequestBody CreateSupplierItemRequest request) {
		return healthCheckService.checkBackendHealth().flatMap(isAvailable -> {
			if (Boolean.TRUE.equals(isAvailable)) {
				return webClientBuilder.build().post()
						.uri(Environment.SERVICE_URL + "supplier-item/create-new-supplier-item")
						.headers(headers -> headers.setBearerAuth(authorizationHeader)).bodyValue(request).retrieve()
	...
	public Mono<ApiResponse<InStockItemResponse>> getInStockItem(@RequestHeader("IniBukanKunci") String authorizationHeader,
			@PathVariable String id) {
		return healthCheckService.checkBackendHealth().flatMap(isAvailable -> {
			if (Boolean.TRUE.equals(isAvailable)) {
				return webClientBuilder.build().get()
						.uri(uriBuilder -> uriBuilder.path(Environment.SERVICE_URL + "instock-item/get-item-by-id")
	...
	public Mono<ApiResponse<String>> addInstockItem(@RequestHeader("IniBukanKunci") String authorizationHeader,
			@RequestBody CreateInStockItemRequest request) {
		UpdateWarehouseItemRequest warehouseRequest = new UpdateWarehouseItemRequest();
		warehouseRequest.setVersion(request.getWarehouseItemVersion());
		warehouseRequest.setQty(request.getWarehouseItemQty());
		...
		return healthCheckService.checkBackendHealth().flatMap(isAvailable -> {
			if (Boolean.TRUE.equals(isAvailable)) {
				return webClientBuilder.build().patch()
						...
						.headers(headers -> headers.setBearerAuth(authorizationHeader)).bodyValue(warehouseRequest).retrieve()
						.bodyToMono(String.class)
						.flatMap(response -> webClientBuilder.build().post()
								.uri(Environment.SERVICE_URL + "instock-item/add-item")
								.headers(headers -> headers.setBearerAuth(authorizationHeader)).bodyValue(request).retrieve()
								.bodyToMono(String.class).flatMap(secondResponse -> {
									ApiResponse<String> apiResponse = new ApiResponse<>();
									ApiResponse.Header header = new ApiResponse.Header();
									header.setRequestId(UUID.randomUUID().toString());
									header.setTimestamp(LocalDateTime.now().toString());
	...
	public Mono<ApiResponse<String>> restockInStockItem(@RequestHeader("IniBukanKunci") String authorizationHeader,
			@RequestBody UpdateInStockItemRequest request) {
		UpdateWarehouseItemRequest warehouseRequest = new UpdateWarehouseItemRequest();
		warehouseRequest.setQty(request.getWarehouseItemQty());
		warehouseRequest.setSupplierItemId(request.getSupplierItemId());
		warehouseRequest.setVersion(request.getVersion());
		return healthCheckService.checkBackendHealth().flatMap(isAvailable -> {
			if (Boolean.TRUE.equals(isAvailable)) {
				return webClientBuilder.build().patch()
						.uri(Environment.SERVICE_URL + "warehouse-item/edit-warehouse-item")
						.headers(headers -> headers.setBearerAuth(authorizationHeader)).bodyValue(warehouseRequest).retrieve()
						.bodyToMono(String.class)
						.flatMap(response -> webClientBuilder.build().patch()
								.uri(Environment.SERVICE_URL + "instock-item/edit-item-quantity")
								.headers(headers -> headers.setBearerAuth(authorizationHeader)).bodyValue(request).retrieve()
								.bodyToMono(String.class).flatMap(secondResponse -> {
									ApiResponse<String> apiResponse = new ApiResponse<>();
									ApiResponse.Header header = new ApiResponse.Header();
